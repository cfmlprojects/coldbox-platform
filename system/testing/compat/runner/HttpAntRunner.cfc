  ...
  <cffunction name="run" access="remote" output="true" returntype="void" hint="Generates and prints HTML, JUnit style XML, or XML data based on a directory of tests.">
    ...
    <cfscript>
      ...
      if(arguments.type is "dir"){
        ...
        var runResult = createObject("component","DirectoryTestSuite").run(directory=arguments.value, componentPath=arguments.componentPath, recurse=arguments.recurse, excludes=arguments.excludes);
        results = runResult.getResults();
      }
    </cfscript>
    <!--- Read by Ant client and used to print summary to stdout --->
    <cfcookie name="mxunit_summary" value="#results.getTotalSpecs()#,#results.getTotalError()#,#results.getTotalFail()#,#results.getTotalDuration()#" />
    <!--- write the cookie first --->
	<cfset reporter = new testbox.system.testing.reports.ANTJUnitReporter() />
	<cfset report = trim(reporter.runReport(results,runResult.tb)) />
	<cfcontent type="text/xml" reset="true">#report#
    <cfreturn />
 </cffunction>
